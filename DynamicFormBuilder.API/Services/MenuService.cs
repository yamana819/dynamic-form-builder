...
public class MenuService : IMenuService
{
    ...
    public async Task CreateMenuForFormGroupAsync(string formGroupCode,string formGroupName)
    {
        Menu menu = new Menu
        {
            ParentMenuId=null,
        ...
        await _authorizationService.CreateAuthorizationsForNewMenuAsync(menu.MenuId);
    }
    public async Task CreateMenuForFormAsync(Guid formId,string formGroupCode,string formName)
    {
        Menu? parentMenu = await _context.Menus
                    .Where(m=>m.Href==$"/forms/{formGroupCode}")
    ...
    {
        Menu? menu = await _context.Menus
                        .Where(m=>m.Href==$"/forms/{formGroupCode}/{formId}")
