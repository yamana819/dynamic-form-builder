...
namespace DynamicFormBuilder.API.Data.Helpers
{
    public class SqlHelper
    {
        ...
        private string SanitizeIdentifier(string identifier)
        {
            ...
            {
                throw new BadRequestException("Bu tanımlayıcı null veya boş.");
            }
            if (!Regex.IsMatch(identifier, "^[a-zA-Z0-9_]+$"))
            {
                throw new BadRequestException("Bu tanımlayıcı kabul edilemez.");
            }
            return $"[{identifier}]";
        }
        ...
        public async Task<int> ExecuteNonQueryAsync(string query, params SqlParameter[] parameters)
        {
            ...
            {
                throw new BadRequestException("Boş query gönderildi.");
            }
        ...
        public async Task<int> InsertRecordFromJson(string tableName, Dictionary<String, object> formData)
        {
            ...
            {
                throw new BadRequestException("Form verisi boş.");
            }
        ...
        public async Task<int> UpdateRecordFromJson(string tableName, string targetPKName, object id, Dictionary<String, object> formData)
        {
            if (formData == null || formData.Count == 0)
            {
                throw new BadRequestException("Form verisi boş.");
        ...
        public async Task<Dictionary<string, object>?> GetRecordByIdAsync(string tableName, string targetPkName, object id)
        {
            string checkedTableName = SanitizeIdentifier(tableName);
        ...
        public async Task<int> DeleteRecordAsync(string tableName, string targetPkName, object id)
        {
            string checkedTableName = SanitizeIdentifier(tableName);
            string checkedPkName = SanitizeIdentifier(targetPkName);
            string pkParameterName = $"@{targetPkName}";

            string query = $"UPDATE {checkedTableName} SET is_deleted = 1 WHERE {checkedPkName} = {pkParameterName} AND is_deleted = 0";

            return await ExecuteNonQueryAsync(query, new SqlParameter(pkParameterName, id));
        }
    }
}